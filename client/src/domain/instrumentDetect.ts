...
import { scrapeMidiProgram, scrapePartTranspose } from './musicxml';
...
export function guessImportInstrument(
  content: string,
  parts: { id: string; name: string | null }[],
): ImportInstrumentGuess {
  ...
  const instrument = mustAskPart ? null : detectInstrument(content, only);
  return {
    instrument,
    mustAskPart,
    mustAskInstrument: instrument === null,
  };
}
